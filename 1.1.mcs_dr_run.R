...
#          curves for the metabolic cell stress endpoint class (oxidative stress
#          and tissue injury markers, e.g. DCFDA, MDA, GSH, GGT, ALT, AST).
#          These endpoints increase monotonically with dose and have no
#          biologically fixed maximum, so a four-parameter Hill function is used,
#          with both the lower (a) and upper (c) asymptote estimated:
#
#              y = a * (1 + (c - 1) * x^d / (b^d + x^d))
#
#          where x = nanoparticle dose, b = potency, d = slope.
#          Fitting is Bayesian (MCMC in R); convergence is checked via R-hat < 1.2.
#          BMD10 (concentration giving a 10% change from the negative control)
#          is extracted per experiment together with its credible interval.
...
plot_fit_initial <- function(x_obs,y_obs){
  hill_function_bc <- function(x, a, b,c, d) {
    # a * (c^(x^d / (b^d + x^d))) hill-ced model 
    a *(1 +(c-1)*(x^d / (b^d + x^d)))
  }
...
generate_pred_curves <- function(draws_df, x_seq, step = 100) {
  ...
  pred_curves <- purrr::map_dfr(draw_indices, function(j) {
    ...
    #y_pred <- a_j * c_j^(x_seq^d_j / (b_j^d_j + x_seq^d_j)) hill-ced model 
...
plot_hill_curve <- function(ribbon_df, ls_bmd_bc, obs_df, study_index, x_lower, x_upper,
                            ec = NULL,
  ...
  p <- ggplot() +
    ...
    labs(
      #title = paste0("Case ", study_index, ": Predictive D-R Curve with EC"),
      title = stringr::str_wrap(paste0(plot_title_begin, " case ", study_index, ": ", 
                    ls_bmd_bc[[study_index]]$`Substance name`[1],"_", 
                    ls_bmd_bc[[study_index]]$`Cell type`[1]), width = 50),
...
check_diagnostics <- function(fit, study_index, parameters = c("a", "b","c","d", "sigma", "ec5"),
                              rhat_threshold = 1.01, ess_threshold = 200) {
  summary_fit <- fit$summary()
  sub <- summary_fit[summary_fit$variable %in% parameters, ]
  
  diag_df <- data.frame(
    study     = study_index,
    parameter = sub$variable,
    rhat      = sub$rhat,
    ess_bulk  = sub$ess_bulk,
    ess_tail  = sub$ess_tail,
    rhat_flag = sub$rhat     > rhat_threshold,
    ess_flag  = sub$ess_bulk < ess_threshold
  )
  
  if (any(diag_df$rhat_flag, na.rm = TRUE))
    message(sprintf("Study %d: Rhat > %.2f for some parameters", study_index, rhat_threshold))
  if (any(diag_df$ess_flag, na.rm = TRUE))
    message(sprintf("Study %d: ESS < %d for some parameters", study_index, ess_threshold))
...
outputdir = "results_undissolved/mouse/plots/bmd_mouse_bc_stan_new"
...
plot_title_begin = "Metabolic cell stress"
...
outputdir = "results_undissolved/human/plots/bmd_human_bc_stan_new"
...
# ------------------ 3. human genototoxicity data unnormalized ------------------
load("/Users/wuji/work/code/codo_v2/R/results/human/ls_sub_case_human.RData")
outputdir = "results/human/plots/bmd_human_geno_stan"
stanoutdir = "/Users/wuji/work/code/codo_v2/R/results/human/stan_results"
...
load("/Users/wuji/work/code/codo_v2/R/results/human/ls_sub_case_human.RData")
outputdir = "results/human/plots/bmd_human_ldh_stan"
stanoutdir = "/Users/wuji/work/code/codo_v2/R/results/human/stan_results"

load("/Users/wuji/work/code/codo_v2/R/results_undissolved/human/ls_sub_case_human.RData")
outputdir = "results_undissolved/human/plots/bmd_human_ldh_stan_wtitle"
stanoutdir = "/Users/wuji/work/code/codo_v2/R/results_undissolved/human/stan_results/bmd_human_ldh_stan"

plot_title_begin = "LDH"
ex_type = "ldh"
col_response = 'Results'
...
r_squared_df <- list()
all_diagnostics <- vector("list", length(ls_bmd_bc))
...
for (i in seq_along(ls_bmd_bc)) {
  ...
  tryCatch({
      # Try loading file
      fit <- readRDS(file.path(stanoutdir, sprintf(outputfile, i)))
    }, error = function(e) {
      message(sprintf("Skipping Study_%d due to error: %s", i, e$message))
    # Skip this iteration
    NULL
    })
  ...
  summary_pars <- summarise_draws(
    ...
    mean, median, sd, mad,
    ~quantile2(.x, probs = c(0.05, 0.25, 0.75, 0.95)),  
  ...
  p <- plot_hill_curve(ribbon_df, ls_bmd_bc, obs_df, i, x_lower, x_upper, 
                       median_ec10,ec10_ci_50[1],ec10_ci_50[2],
                       ec10_ci_95[1],ec10_ci_95[2],
                       yaxis_label=capitalize_if_needed(y_axis_title))
  p
  dir.create(dirname(file.path(outputdir, sprintf("CASE_%d_curve.pdf", i))), recursive = TRUE, showWarnings = FALSE)
  
  if (abs(log10(median_ec10) - log10(ec10_ci_95[2])) > 3) {
    # Save plot
    ggsave(
      file.path(outputdir, sprintf("uncertain/CASE_%d_curve.pdf", i)),
      plot = p, width = 8, height = 5,device = cairo_pdf)}else{  # Save plot
    ggsave(
         file.path(outputdir, sprintf("CASE_%d_curve.pdf", i)),
...
save(diagnostics_df, file = file.path(stanoutdir,"bc_mouse_diagnostics.RData"))
...
save(diagnostics_df, file = file.path(stanoutdir,"bc_human_diagnostics.RData"))
...
save(diagnostics_df, file = file.path(stanoutdir,"geno_human_diagnostics.RData"))
...
save(diagnostics_df, file = file.path(stanoutdir,"ldh_human_diagnostics.RData"))
